...
if(empty($_POST['rating']) || empty($_POST['subject'])) {
    $e=mysqli_error($con);
    header("location: electricalcourse.php?remarks=failed");
} else if  (mysqli_query($con, $sql)) {
    // echo "THANKS FOR YOUR FEEDBACK !!!:)";
    // echo "Redirecting in 2s";
    header("location:electricalcourse.php?remarks=success");
} else {
    header("location: electricalcourse.php?remarks=error");
}
